...
def buscar_endereco_via_cep(cep: str) -> dict:
    """Busca o endereço via CEP. Retorna um dicionário vazio em caso de erro."""
    cep_limpo = re.sub(r'\D', '', cep)  # Remove todos os caracteres não numéricos
    if not cep_limpo or len(cep_limpo) != 8:  # Verifica se o CEP tem 8 dígitos
        return {}
    try:
