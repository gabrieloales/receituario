...
def gerar_pdf_receita(
        ...
        imagem_assinatura="assinatura_isa.png",
        nome_vet="M. V. ISABELA ZAMBONI",
        crmv="CRVM-PR: 22845"
):
    ...
    # Posições para a assinatura e campos no rodapé
    config_posicoes = {
    ...
    for i, med in enumerate(lista_medicamentos, start=1):
        ...
        if conc:
            ...
            # Imprime os dados de concentração conforme digitados (sem forçar caixa alta)
    ...
    c.drawCentredString(x_assinatura, config_posicoes["mv_y"], nome_vet)
    c.drawCentredString(x_assinatura, config_posicoes["crmv_y"], crmv)
...
def criar_receita():
    ...
    if eh_controlado:
        ...
        if forma_end == "2":
            ...
            endereco_formatado = f"{rua}, {numero_auto} - {bairro} - {cidade} - {estado} - Brasil - CEP: {formatar_cep(cep_auto)}"
        else:
            rua = input("Rua: ").strip()
            ...
            endereco_formatado = f"{rua}, {numero} - {bairro} - {cidade} - {estado} - Brasil - CEP: {formatar_cep(cep_manual)}"
